"""
VAE Classifier v1.1 - Hierarchical Classification

Supervised neural network classifier using v2.6.7 VAE latent embeddings.

Changes from v1.0:
- HIERARCHICAL CLASSIFICATION: 14 lithology groups instead of 139 fine-grained classes
...
This addresses extreme class imbalance by grouping similar lithologies:
- 139 fine-grained classes → 14 geologically meaningful groups
- Reduces imbalance ratio from 49,778:1 to ~150:1
"""
...
def train_classifier(model, train_loader, val_loader, epochs, device, criterion, lr=1e-3):
    ...
    for epoch in range(epochs):
        ...
        if val_balanced_acc > best_val_balanced_acc:
            ...
            patience_counter = 0
            # Save best model
            torch.save(model.state_dict(), 'ml_models/checkpoints/vae_classifier_v1_1_best.pth')
        else:
            patience_counter += 1
    ...
    # Load best model
    model.load_state_dict(torch.load('ml_models/checkpoints/vae_classifier_v1_1_best.pth'))

    return model

print("="*100)
print("VAE CLASSIFIER v1.1 (HIERARCHICAL + CLASS-BALANCED LOSS + ENTROPY-BALANCED SPLIT)")
print("="*100)
print()
...
# Map Principal lithologies to groups
df['Lithology_Group'] = df['Principal'].map(principal_to_group)

# Check for unmapped lithologies
unmapped = df[df['Lithology_Group'].isna()]
if len(unmapped) > 0:
    print(f"WARNING: {len(unmapped)} samples with unmapped lithologies:")
    print(unmapped['Principal'].value_counts())
    df = df[df['Lithology_Group'].notna()]  # Remove unmapped samples

feature_cols = ['Bulk density (GRA)', 'Magnetic susceptibility (instr. units)',
                'NGR total counts (cps)', 'R', 'G', 'B']

X = df[feature_cols].values
y = df['Lithology_Group'].values  # Use groups instead of Principal

print(f"Total samples: {len(df):,}")
print(f"Total boreholes: {df['Borehole_ID'].nunique()}")
print(f"Unique fine-grained lithologies: {df['Principal'].nunique()}")
print(f"Unique lithology groups: {len(np.unique(y))}")
print()

# Show group distribution
group_counts = pd.Series(y).value_counts()
print("Lithology group distribution:")
for group, count in group_counts.items():
    pct = 100 * count / len(y)
    print(f"  {group:30s}: {count:6,d} samples ({pct:5.2f}%)")
print()
...
print(f"Number of classes (groups): {len(unique_labels)}")
print()
...
print()
print("="*100)
print(f"✓ VAE Classifier v1.1 trained successfully")
print(f"  Test Accuracy (unweighted): {test_acc*100:.2f}%")
print(f"  Test Accuracy (balanced):   {balanced_acc*100:.2f}%")
print(f"  Model saved: ml_models/checkpoints/vae_classifier_v1_1_best.pth")
print("="*100)
