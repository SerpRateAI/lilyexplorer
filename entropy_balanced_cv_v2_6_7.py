"""
Entropy-balanced cross-validation for v2.6.7 (β: 1e-10 → 0.75).

This tests if the improved β schedule (1e-10 → 0.75) discovered on the lucky split
holds up when evaluated with entropy-balanced stratified CV.

Strategy:
1. Stratify by dominant lithology to ensure similar entropy across folds
2. Run 5-fold CV with new β schedule
3. Compare to v2.6.6 baseline (ARI = 0.19 ± 0.05)
"""
...
print("="*100)
print("ENTROPY-BALANCED CROSS-VALIDATION - v2.6.7")
print("="*100)
...
print("="*100)
print("COMPARISON TO v2.6.6")
print("="*100)
print(f"v2.6.6 (β: 0.001→0.5):         ARI = 0.190 ± 0.045")
print(f"v2.6.7 (β: 1e-10→0.75):         ARI = {mean_ari:.4f} ± {std_ari:.4f}")
print()

improvement = ((mean_ari - 0.190) / 0.190) * 100
print(f"Improvement: {improvement:+.1f}%")
print()

if mean_ari > 0.190:
    print("✓ v2.6.7 β schedule improves performance on entropy-balanced CV!")
else:
    print("⚠️ v2.6.7 β schedule does not improve vs v2.6.6 on balanced folds")
    print("   Lucky split improvement may not generalize")

print("="*100)

df_cv.to_csv('v2_6_7_entropy_balanced_cv.csv', index=False)
