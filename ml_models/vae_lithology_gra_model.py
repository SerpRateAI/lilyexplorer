"""
VAE Lithology Model using GRA + MS + NGR measurements with depth binning.

This model uses 403K+ samples from 524 boreholes with 3 MSCL measurements:
...
- 2D and 8D latent space models for comparison
- Borehole-level train/val/test split (no data leakage)
...
def main():
    ...
    print("="*80)
    print("VAE LITHOLOGY MODEL - GRA + MS + NGR")
    print("="*80)

    # Configuration
    data_path = Path('/home/utig5/johna/bhai/vae_training_data_20cm.csv')
    ...
    for latent_dim in latent_dims:
        ...
        # Create model
        model = VAE(input_dim=3, latent_dim=latent_dim, hidden_dims=[16, 8]).to(device)
        ...
        # Save model
        model_path = checkpoint_dir / f'vae_gra_latent{latent_dim}.pth'
        torch.save({
        ...
        axes[0].set_title(f'VAE Loss (Latent Dim={latent_dim})')
        axes[0].legend()
        axes[0].grid(True)
        ...
        plt.tight_layout()
        plt.savefig(output_dir / f'training_history_latent{latent_dim}.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        if latent_dim == 2:
            ...
            colors = [lith if lith in top_lithologies.values else 'Other' for lith in lith_test]
            ...
            for ax, data, title in [(axes[0], (latent_train, lith_train), 'Train'),
                ...
                ax.set_title(f'2D Latent Space - {title}')
                ax.legend(markerscale=3, fontsize=8)
                ax.grid(True, alpha=0.3)

            plt.tight_layout()
            plt.savefig(output_dir / 'latent_space_2d.png', dpi=300, bbox_inches='tight')
            plt.close()
            ...
            for ax, data, title in zip(axes, projections, titles):
                ...
                ax.set_title(f'8D Latent Space - {title}')
                ax.legend(markerscale=3, fontsize=8)
                ax.grid(True, alpha=0.3)

            plt.tight_layout()
            plt.savefig(output_dir / 'latent_space_8d.png', dpi=300, bbox_inches='tight')
            plt.close()
