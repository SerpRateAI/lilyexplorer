"""
VAE Lithology Model v2.1 - GRA + MS + NGR + RGB with Distribution-Aware Scaling

This model improves upon v2 with feature-specific preprocessing based on distributions:
- GRA bulk density: Gaussian → StandardScaler (unchanged)
- Magnetic susceptibility: Poisson → log(x+1) + StandardScaler
- NGR: Bimodal → log(x+1) + StandardScaler
...
def main():
    ...
    print("="*80)
    print("VAE LITHOLOGY MODEL v2.1 - Distribution-Aware Scaling")
    print("="*80)

    # Configuration
    data_path = Path('/home/utig5/johna/bhai/vae_training_data_v2_20cm.csv')
    output_dir = Path('/home/utig5/johna/bhai/vae_v2_1_outputs')
    ...
    for latent_dim in latent_dims:
        ...
        # Save model
        model_path = checkpoint_dir / f'vae_gra_v2_1_latent{latent_dim}.pth'
        torch.save({
        ...
        axes[0].set_title(f'VAE v2.1 Loss (Latent Dim={latent_dim})')
        axes[0].legend()
        axes[0].grid(True)
        ...
        plt.tight_layout()
        plt.savefig(output_dir / f'training_history_v2_1_latent{latent_dim}.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        if latent_dim == 2:
            ...
            for ax, data, title in [(axes[0], (latent_train, lith_train), 'Train'),
                ...
                ax.set_title(f'2D Latent Space v2.1 - {title}')
                ax.legend(markerscale=3, fontsize=8)
                ax.grid(True, alpha=0.3)

            plt.tight_layout()
            plt.savefig(output_dir / 'latent_space_v2_1_2d.png', dpi=300, bbox_inches='tight')
            plt.close()
            ...
            for ax, data, title in zip(axes, projections, titles):
                ...
                ax.set_title(f'8D Latent Space v2.1 - {title}')
                ax.legend(markerscale=3, fontsize=8)
                ax.grid(True, alpha=0.3)

            plt.tight_layout()
            plt.savefig(output_dir / 'latent_space_v2_1_8d.png', dpi=300, bbox_inches='tight')
            plt.close()
