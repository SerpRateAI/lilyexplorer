"""
VAE Lithology Model v2.5 - Low β for Improved Clustering (β=0.01)

Key change from v2.1: Reduces β from 1.0 to 0.01 to minimize disentanglement.

Literature finding: Disentanglement HARMS clustering performance [Rasmussen et al.]
- Standard VAE (β=1.0) forces independence between latent dimensions
- Geological features are naturally correlated (e.g., MS ↔ alteration)
- β=0.01 preserves feature correlations → better clustering
- Studies show +0.583 ARI improvement with β=0.01 vs β=1.0

Same preprocessing as v2.1:
...
def main():
    ...
    print("="*80)
    print("VAE LITHOLOGY MODEL v2.5 - Low β (β=0.01) for Better Clustering")
    print("="*80)

    # Configuration
    data_path = Path('/home/utig5/johna/bhai/vae_training_data_v2_20cm.csv')
    output_dir = Path('/home/utig5/johna/bhai/vae_v2_5_outputs')
    ...
    # KEY CHANGE: Use beta=0.01 instead of default 1.0
    beta = 0.01
    print(f"Using β={beta} (low disentanglement for clustering)\n")
    ...
    for latent_dim in latent_dims:
        ...
        # Train with beta=0.01
        start_time = time.time()
        model, history = train_vae(model, train_loader, val_loader, epochs=100, device=device, beta=beta)
        train_time = time.time() - start_time
        print(f"Training time: {train_time:.1f}s")

        # Save model
        model_path = checkpoint_dir / f'vae_gra_v2_5_latent{latent_dim}.pth'
        torch.save({
        ...
        axes[0].set_title(f'VAE v2.5 (β=0.01) Loss (Latent Dim={latent_dim})')
        axes[0].legend()
        axes[0].grid(True)
        ...
        plt.tight_layout()
        plt.savefig(output_dir / f'training_history_v2_5_latent{latent_dim}.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        if latent_dim == 2:
            ...
            for ax, data, title in [(axes[0], (latent_train, lith_train), 'Train'),
                ...
                ax.set_title(f'2D Latent Space v2.5 (β=0.01) - {title}')
                ax.legend(markerscale=3, fontsize=8)
                ax.grid(True, alpha=0.3)

            plt.tight_layout()
            plt.savefig(output_dir / 'latent_space_v2_5_2d.png', dpi=300, bbox_inches='tight')
            plt.close()
            ...
            for ax, data, title in zip(axes, projections, titles):
                ...
                ax.set_title(f'8D Latent Space v2.5 (β=0.01) - {title}')
                ax.legend(markerscale=3, fontsize=8)
                ax.grid(True, alpha=0.3)

            plt.tight_layout()
            plt.savefig(output_dir / 'latent_space_v2_5_8d.png', dpi=300, bbox_inches='tight')
            plt.close()
