...
Uses ENTROPY-BALANCED borehole splitting to maintain similar lithology distributions
across train/val/test while preventing spatial leakage.
"""
...
def entropy_based_split(df, train_frac=0.70, val_frac=0.15, test_frac=0.15, random_state=42):
    ...
    Uses greedy allocation sorted by borehole size to balance distributions.
    """
...
def train_classifier(model, train_loader, val_loader, epochs, device, lr=1e-3):
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_val_acc:
            ...
            patience_counter = 0
            # Save best model
            torch.save(model.state_dict(), 'ml_models/checkpoints/lithology_classifier_v2_6_7_entropy_balanced_best.pth')
        else:
            patience_counter += 1
    ...
    # Load best model
    model.load_state_dict(torch.load('ml_models/checkpoints/lithology_classifier_v2_6_7_entropy_balanced_best.pth'))

    return model

print("="*100)
print("LITHOLOGY CLASSIFIER USING v2.6.7 VAE EMBEDDINGS (ENTROPY-BALANCED SPLIT)")
print("="*100)
print()
...
print(f"  Test Accuracy (unweighted): {test_acc*100:.2f}%")
print(f"  Test Accuracy (balanced):   {balanced_acc*100:.2f}%")
print(f"  Model saved: ml_models/checkpoints/lithology_classifier_v2_6_7_entropy_balanced_best.pth")
print("="*100)
