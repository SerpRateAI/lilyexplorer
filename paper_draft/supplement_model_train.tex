...
\item Text S7: Cross-Validation and Performance Variance
\item Text S8: Extreme $\beta$ Annealing (v2.6.7)
\item Text S9: Multi-Decoder Architecture (v2.13)
\item Text S10: Semi-Supervised VAE (v2.14)
...
\item Table S5: Entropy-Balanced Cross-Validation Results (v2.6.6)
\item Table S6: Entropy-Balanced Cross-Validation Results (v2.6.7)
\item Table S7: $\beta$ Grid Search for Multi-Decoder VAE (v2.13)
\item Table S8: Semi-Supervised VAE $\alpha$ Grid Search (v2.14)
...
\item Figure S7: Multi-Decoder Architecture Diagram (v2.13)
\end{enumerate}
...
\subsection*{S2.5 Fuzzy Depth Matching (v2.6.8)}

\textbf{Motivation.} v2.6.7 uses exact 20~cm depth binning to co-locate GRA, MS, NGR, and RGB measurements. This strict alignment requirement may discard samples where measurements are slightly offset. Hypothesis: relaxing depth matching tolerance could extract more training samples from the existing 296 boreholes with RGB coverage.

\textbf{Method.} Fuzzy matching: for each 20~cm bin center, accept measurements within $\pm$20~cm tolerance. Take closest measurement within tolerance for each feature type. This allows measurements up to 40~cm apart to be paired (one at $+$20~cm, another at $-$20~cm from bin center).

\textbf{Implementation.} Created fuzzy matched dataset: 251,210 samples (+5.3\% vs v2.6.7's 238,506) from same 296 boreholes. Trained v2.6.8 model with identical architecture to v2.6.7: 10D latent, $\beta$ annealing $10^{-10} \rightarrow 0.75$ over 50 epochs, distribution-aware scaling.

\textbf{Results.} Training converged normally (18 epochs, 100.6~s). Clustering performance catastrophically degraded:
\begin{itemize}
\item $k=10$: ARI = 0.087 ($-$55.5\% vs v2.6.7's 0.196)
\item $k=12$: ARI = 0.086 ($-$56.1\% vs v2.6.7)
\item Best ARI: 0.087 at $k=10$ (worst of all v2.6.x variants)
\end{itemize}

\textbf{Root cause: Label noise from relaxed alignment.} At lithological boundaries, fuzzy matching pairs measurements from different rock types. Example: ``clay'' measurement at 10.0~m paired with ``basalt'' at 10.4~m across contact. Up to 40~cm separation allows mixing features from distinct lithologies. Model learns spurious feature-lithology associations instead of true signatures.

\textbf{Quantifying noise.} Of 12,676 additional samples (+5.3\%), estimated $\sim$30--40\% cross lithological boundaries (based on average contact spacing $\sim$1~m in sedimentary sections). This introduces $\sim$1,500--2,000 mislabeled training examples, enough to severely degrade learned representations.

\textbf{Silhouette scores paradox.} Despite catastrophic ARI degradation, silhouette scores remained moderate (0.146--0.154). This indicates model still learns well-separated clusters in latent space, but these clusters no longer align with true lithology due to corrupted training labels. Well-separated but wrong clusters.

\textbf{Conclusion.} Data quality dominates data quantity for clustering. v2.6.7's exact 20~cm binning is optimal: adding 5.3\% noisy samples destroys 55\% of clustering performance. Strict depth alignment preserves lithological coherence. Future work should focus on improving feature quality (e.g., higher-resolution imaging) rather than relaxing matching constraints.

\subsection*{S2.6 Wider Architecture (v2.12)}

\textbf{Motivation.} v2.6.7 uses shallow encoder/decoder architecture [32, 16] with only 2,010 parameters. Hypothesis: wider, deeper architecture [256, 128, 64, 32] with 45$\times$ more parameters (91,034) can learn richer representations for improved clustering.

\textbf{Architecture.} Encoder: 6D$\rightarrow$[256, 128, 64, 32]$\rightarrow$10D latent. Decoder: 10D$\rightarrow$[32, 64, 128, 256]$\rightarrow$6D. Parameters: 91,034 vs v2.6.7's 2,010 (+4450\%). Same training dataset: 238,506 samples, 296 boreholes, filtered $\geq$100 samples per class.

\textbf{Tested configurations.}
\begin{itemize}
\item $\beta = 0.1$, 100 epochs: ARI = 0.1212, 4/10 active dims, 625~s training
\item $\beta = 0.75$, 200 epochs: ARI = 0.1289, 3/10 active dims, 1244~s training
\end{itemize}

\textbf{Results.} Best v2.12: ARI = 0.1289 ($-$34.2\% vs v2.6.7's 0.196). Despite 45$\times$ more parameters and 2$\times$ training epochs, performance catastrophically degraded.

\textbf{Critical issues.}
\begin{enumerate}
\item \textbf{Posterior collapse:} Only 3--4/10 latent dimensions active (vs v2.6.7's $\sim$4/10 with superior clustering)
\item \textbf{$\beta$ sensitivity:} $\beta=0.75$ causes more severe collapse in wider model (3/10 dims) vs gentler $\beta=0.1$ (4/10 dims)
\item \textbf{Overfitting:} 45$\times$ parameters don't improve generalization despite extended training
\item \textbf{Training cost:} 2$\times$ longer for 34\% worse performance
\end{enumerate}

\textbf{Initial bug - full-batch training.} First attempt trained on entire 202K sample dataset as single batch (not mini-batch SGD), completing in 4.2~s with ARI = 0.106. This represented only 100 total gradient updates, not proper stochastic training. Bug detected when training was suspiciously fast despite poor performance. Fixed with proper DataLoader (batch\_size = 256, 792 batches/epoch).

\textbf{Why capacity $\neq$ performance.} The wider architecture has too much capacity:
\begin{itemize}
\item \textbf{Bottleneck benefit:} v2.6.7's tight [32, 16] architecture forces information compression, learning discriminative features
\item \textbf{Overparameterization:} 91K parameters enable memorizing spurious patterns instead of generalizable lithology signatures
\item \textbf{Regularization failure:} $\beta$ schedule effective for shallow model causes collapse in deep model
\item \textbf{No depth benefit:} 4 layers don't capture additional geological structure vs 2 layers
\end{itemize}

\textbf{Conclusion.} \textbf{Architectural simplicity is a feature, not a limitation.} The shallow [32, 16] bottleneck forces beneficial information compression. Similar to ResNet-50 often outperforming ResNet-200, architectural constraints provide inductive bias. Wider models learn less useful representations despite greater capacity. v2.6.7's architecture is optimal for this task.
...
\section*{Text S9: Multi-Decoder Architecture (v2.13)}

After v2.6.7 established optimal $\beta$ annealing (1e-10 $\rightarrow$ 0.75), we investigated whether architectural modifications could improve reconstruction quality without compromising clustering performance.

\subsection*{S9.1 Motivation: Heteroskedastic Reconstruction}

Analysis of v2.6.7 reconstruction revealed heteroskedastic patterns: different lithology groups exhibit dramatically different reconstruction errors for the same feature. Example MS (magnetic susceptibility):
\begin{itemize}
\item Basalt: $R^2 = -0.02$ (poor reconstruction)
\item Clay: $R^2 = 0.58$ (moderate reconstruction)
\item Nannofossil ooze: $R^2 = 0.62$ (moderate)
\end{itemize}

Standard single-decoder VAE must balance reconstruction quality across all features and lithologies simultaneously, potentially under-fitting some features.

\subsection*{S9.2 Multi-Decoder Architecture}

\textbf{Design:} Shared encoder, separate decoder per feature.

\textbf{Architecture comparison:}
\begin{itemize}
\item \textbf{v2.6.7 (single decoder):} Encoder 6D $\rightarrow$ [32, 16] $\rightarrow$ 10D latent. Decoder 10D $\rightarrow$ [16, 32] $\rightarrow$ 6D output. Parameters: 2,010.
\item \textbf{v2.13 (multi-decoder):} Encoder 6D $\rightarrow$ [32, 16] $\rightarrow$ 10D latent. Six separate decoders: GRA, MS, NGR, R, G, B each 10D $\rightarrow$ [16, 32] $\rightarrow$ 1D. Parameters: 5,610 (+179\%).
\end{itemize}

Each decoder specializes in reconstructing one feature, potentially learning feature-specific nonlinearities.

\textbf{Feature weighting:} Loss function applies weights [1.0, 2.0, 2.0, 1.0, 1.0, 1.0] to GRA, MS, NGR, R, G, B respectively. 2$\times$ weight on MS and NGR reflects their importance for lithology discrimination and poorer baseline reconstruction quality.

\subsection*{S9.3 Cross-Validation Results}

Entropy-balanced 5-fold CV (same methodology as v2.6.7, see Text S7):

\textbf{Clustering performance:}
\begin{itemize}
\item v2.13: ARI = 0.186 $\pm$ 0.024 (95\% CI: [0.139, 0.233])
\item v2.6.7: ARI = 0.196 $\pm$ 0.037 (95\% CI: [0.124, 0.268])
\item Difference: $-$5.1\% (confidence intervals overlap)
\end{itemize}

\textbf{Conclusion:} Statistically equivalent clustering performance with lower variance.

\textbf{Reconstruction quality (measured on full dataset):}
\begin{itemize}
\item GRA: $R^2 = 0.69$
\item MS: $R^2 = 0.48$ ($\sim$9\% vs estimated v2.6.7 baseline of 0.44)
\item NGR: $R^2 = 0.72$
\item R: $R^2 = 0.89$
\item G: $R^2 = 0.89$
\item B: $R^2 = 0.87$
...
Multi-decoder achieves \textbf{same clustering quality} as v2.6.7 while maintaining comparable reconstruction with added decoder flexibility.

\subsection*{S9.4 $\beta$ Optimization for Multi-Decoder}

Grid search over $\beta_{\text{end}} \in \{0.5, 0.75, 1.0, 1.5, 2.0\}$ with fixed $\beta_{\text{start}} = 10^{-10}$, annealing over 50 epochs. Each $\beta$ tested with 5-fold entropy-balanced CV (25 total training runs). Results (Table S7):

\begin{itemize}
\item $\beta = 0.5$: ARI = 0.177 $\pm$ 0.028
\item $\beta = 0.75$: ARI = 0.186 $\pm$ 0.025 $\leftarrow$ baseline (lowest variance)
\item $\beta = 1.0$: ARI = 0.185 $\pm$ 0.025
\item $\beta = 1.5$: ARI = 0.187 $\pm$ 0.039
\item $\beta = 2.0$: ARI = 0.190 $\pm$ 0.034 $\leftarrow$ highest mean
\end{itemize}

All five $\beta$ values have completely overlapping confidence intervals. Multi-decoder architecture is robust to $\beta$ choice in 0.5--2.0 range. We retain $\beta_{\text{end}} = 0.75$ for consistency with v2.6.7 and lowest variance.

\subsection*{S9.5 Training Efficiency Trade-offs}

\begin{itemize}
\item Training time: 1,471~s (100 epochs) vs v2.6.7's 580~s (+153\%, due to 2.8$\times$ more parameters)
\item Convergence: Early stopping at epoch 16 (same as v2.6.7)
\item GPU memory: Negligible increase (5.6K vs 2.0K parameters, both $\ll$ GPU capacity)
\end{itemize}

\subsection*{S9.6 Scientific Interpretation}

\textbf{Key finding:} Clustering quality is determined by latent space structure (encoder + $\beta$ schedule), not decoder architecture. Multi-decoder and single-decoder achieve equivalent ARI because:

\begin{enumerate}
\item Encoder learns same compressed 10D representation (identical architecture)
\item $\beta$ schedule enforces same KL regularization (1e-10 $\rightarrow$ 0.75)
\item GMM clustering operates on latent $z$, never sees decoder outputs
\item Decoder only affects reconstruction loss gradient, which is dominated by KL loss at $\beta=0.75$
\end{enumerate}

\textbf{Practical implication:} Use v2.6.7 (single decoder) for clustering applications. Use v2.13 (multi-decoder) when high-quality reconstruction is needed (e.g., data imputation, quality assessment, visualization of latent space via reconstruction).

\textbf{Failed alternative:} Gaussian Mixture VAE (GMVAE) with discrete latent components ($K \in \{3, 5, 7, 10\}$) was tested. Results: ARI degraded from 0.19 to 0.02--0.08 despite $R^2 \rightarrow 0.99$. Components collapsed to lookup tables (overfitting) rather than semantic lithology groups. Multi-decoder architecture addresses heteroskedasticity without introducing discrete latent structure.

%% ------------------------------------------------------------------------ %%
%  TEXT S10
%% ------------------------------------------------------------------------ %%

\section*{Text S10: Semi-Supervised VAE (v2.14)}

After confirming that v2.6.7 and v2.13 achieve similar unsupervised clustering performance (ARI $\approx$ 0.19), we investigated whether lithology labels could guide latent space organization to improve clustering beyond the unsupervised ceiling.

\subsection*{S10.1 Motivation}

All previous models are fully unsupervised: trained to reconstruct input features without using lithology labels. Performance plateaued at ARI $\approx$ 0.19--0.20 despite architectural variations (single vs multi-decoder), hyperparameter optimization ($\beta$ annealing), and latent dimensionality tuning (8D vs 10D).

\textbf{Hypothesis:} The challenge is not the clustering algorithm (GMM works well), but learning the right latent representation. If lithology labels guide encoder training, the latent space should better separate lithology groups, improving downstream unsupervised clustering.

\subsection*{S10.2 Architecture}

\textbf{Base:} v2.6.7 architecture (6D input, encoder [32, 16], 10D latent, decoder [16, 32], 6D output).

\textbf{Classification head (NEW):} Branches from 10D latent space:
\begin{itemize}
\item Linear(10$\rightarrow$32)
\item ReLU
\item Dropout(0.2)
\item Linear(32$\rightarrow$139) for 139 lithology classes
\end{itemize}

Parameters: 6,949 total (vs 2,010 for v2.6.7). Additional 4,939 parameters all in classification head.

\subsection*{S10.3 Three-Part Loss Function}

\begin{equation}
\mathcal{L}_{\text{total}} = \mathcal{L}_{\text{recon}} + \beta \cdot \mathcal{L}_{\text{KL}} + \alpha \cdot \mathcal{L}_{\text{class}}
\end{equation}

where:
\begin{itemize}
\item $\mathcal{L}_{\text{recon}}$: MSE between input and reconstructed output
\item $\mathcal{L}_{\text{KL}}$: KL divergence regularization ($\beta$: 1e-10 $\rightarrow$ 0.75, same as v2.6.7)
\item $\mathcal{L}_{\text{class}}$: Cross-entropy for 139-class lithology classification (NEW)
\item $\alpha$: Classification weight (grid search parameter)
\end{itemize}

\subsection*{S10.4 Training Details}

Dataset: 238,506 samples, 296 boreholes (same as v2.6.7). Split: 80\% train / 10\% val / 10\% test (borehole-level, no leakage). Training: 100 epochs, batch size 1,024, Adam optimizer (lr=1e-3). Device: GPU (CUDA).

Grid search over $\alpha \in \{0.01, 0.1, 0.5, 1.0, 2.0\}$ with fixed $\beta$ annealing.

\subsection*{S10.5 Results}

All $\alpha$ values dramatically improved clustering over unsupervised baseline (Table S8):

\begin{itemize}
\item \textbf{$\alpha = 0.1$ (BEST):} ARI = 0.285 (+45.6\% vs v2.6.7's 0.196)
\item $\alpha = 0.01$: ARI = 0.248 (+26.5\%)
\item $\alpha = 0.5$: ARI = 0.232 (+18.4\%)
\item $\alpha = 1.0$: ARI = 0.250 (+27.6\%)
\item $\alpha = 2.0$: ARI = 0.220 (+12.2\%)
\end{itemize}

\textbf{Key observations:}
\begin{enumerate}
\item \textbf{Sweet spot at $\alpha=0.1$:} Gentle classification guidance without overpowering reconstruction. Balances all three loss components.
\item \textbf{Too much supervision hurts:} $\alpha=2.0$ worse than $\alpha=0.1$ despite higher classification accuracy ($\sim$54\% vs $\sim$45\%). Model overfits to classification task, losing good clustering structure.
\item \textbf{Trade-off relationship:} Classification accuracy increases monotonically with $\alpha$, but clustering performance peaks at $\alpha=0.1$ then declines.
\item \textbf{No posterior collapse:} All 10 latent dimensions remain active (std $>$ 0.01) across all $\alpha$ values. Latent space utilization healthy.
\end{enumerate}

\subsection*{S10.6 Why It Works}

\textbf{Supervised guidance organizes latent space.} Classification loss encourages lithologically similar samples to cluster in latent space. GMM benefits from better-separated clusters without needing labels during clustering.

\textbf{Low $\alpha$ preserves reconstruction quality.} Prevents overfitting to classification. Maintains smooth latent manifold needed for unsupervised clustering.

\textbf{Validates hypothesis.} Main challenge was learning good representation (encoder), not clustering algorithm (GMM). Labels guide encoder to learn better latent structure.

\textbf{Philosophy: Guided representation learning.} We use labels during training to learn better encoder, then evaluate clustering with unsupervised GMM (no labels). Fundamentally different from supervised classification.

\subsection*{S10.7 Limitations and Future Work}

\textbf{Limitations:}
\begin{itemize}
\item Requires lithology labels (not fully unsupervised)
\item Single train-test split (needs cross-validation)
\item Supervised bias during training
\item Higher computational cost (+3.5$\times$ parameters)
\end{itemize}

\textbf{Future directions:}
\begin{itemize}
\item \textbf{$\alpha$ annealing (high priority):} Start with pure autoencoder ($\alpha=0$) for 50 epochs to learn reconstruction, then gradually introduce classification guidance. May improve over fixed $\alpha=0.1$.
\item \textbf{5-fold cross-validation:} Confirm +45.6\% improvement holds across splits
\item \textbf{Hierarchical classification:} Use lithology hierarchy (major groups $\rightarrow$ subgroups $\rightarrow$ specific types) instead of flat 139 classes
\item \textbf{UMAP comparison:} Visualize v2.6.7 vs v2.14 latent spaces to understand what supervision changes
\end{itemize}

\subsection*{S10.8 Recommendation}

\textbf{If lithology labels available:} Use semi-supervised v2.14 ($\alpha=0.1$) for +45.6\% clustering improvement.

\textbf{For fully unsupervised:} Use v2.6.7 ($\beta$: 1e-10 $\rightarrow$ 0.75, ARI = 0.196 $\pm$ 0.037).

Both represent fundamentally different approaches to the same goal: learning latent representations that enable lithology clustering from physical property measurements.
...
v2.6.7 & 10D ($\beta$: 1e-10$\rightarrow$0.75) & 239K & 296 & 0.196 $\pm$ 0.037$^*$ & Best (unsupervised) \\
v2.13 & Multi-decoder, 6 decoders & 239K & 296 & 0.186 $\pm$ 0.024$^*$ & Best (reconstruction) \\
\textbf{v2.14} & \textbf{Semi-supervised ($\alpha$=0.1)} & \textbf{239K} & \textbf{296} & \textbf{0.285$^\dagger$} & \textbf{Best (overall)} \\
\midrule
...
v2.6.8 & 10D fuzzy $\pm$20cm & 251K & 296 & 0.087$^\dagger$ & $-$55\%, +5\% data \\
\midrule
...
\begin{table}[h]
\caption{$\beta$ Grid Search for Multi-Decoder VAE (v2.13)}
\centering
\begin{tabular}{@{}cccccc@{}}
\toprule
$\beta_{\text{end}}$ & Mean ARI & Std & Min & Max & Status \\
\midrule
0.50 & 0.1767 & 0.0283 & 0.1459 & 0.2205 & Low variance \\
\textbf{0.75} & \textbf{0.1862} & \textbf{0.0246} & \textbf{0.1621} & \textbf{0.2122} & \textbf{Baseline (lowest $\sigma$)} \\
1.00 & 0.1852 & 0.0248 & 0.1596 & 0.2233 & \\
1.50 & 0.1865 & 0.0388 & 0.1517 & 0.2419 & High variance \\
2.00 & 0.1898 & 0.0335 & 0.1462 & 0.2322 & Highest mean \\
\bottomrule
\end{tabular}
\vspace{0.5em}

\small
\textbf{Methodology:} Grid search over $\beta_{\text{end}} \in \{0.5, 0.75, 1.0, 1.5, 2.0\}$ with fixed $\beta_{\text{start}} = 10^{-10}$, linear annealing over 50 epochs, 100 total epochs. Each $\beta$ tested with 5-fold entropy-balanced CV (same folds as v2.6.7). Total: 25 training runs (5 $\beta$ × 5 folds).

\textbf{Key findings:}
\begin{itemize}
\item All confidence intervals completely overlap: $\beta = 0.5$ (95\% CI: [0.120, 0.233]), $\beta = 0.75$ ([0.137, 0.235]), $\beta = 1.0$ ([0.136, 0.234]), $\beta = 1.5$ ([0.110, 0.264]), $\beta = 2.0$ ([0.123, 0.256])
\item Multi-decoder architecture robust to $\beta$ choice in 0.5--2.0 range (±6\% variation in mean)
\item $\beta = 0.75$ selected for lowest variance (0.0246) and consistency with v2.6.7
\item Higher $\beta$ ($>$1.5) increases variance without improving mean performance
\item Contrast with v2.6.7 single-decoder: optimal at $\beta = 0.75$, degraded at $\beta = 1.0$ ($-$21\%). Multi-decoder less sensitive to $\beta$.
\end{itemize}

\textbf{Comparison to v2.6.7 (single decoder):}
\begin{itemize}
\item v2.6.7 ($\beta$: 1e-10 $\rightarrow$ 0.75): ARI = 0.196 $\pm$ 0.037
\item v2.13 ($\beta$: 1e-10 $\rightarrow$ 0.75): ARI = 0.186 $\pm$ 0.024
\item Difference: $-$5.1\% (overlapping CI, statistically equivalent with lower variance)
\end{itemize}

\textbf{Conclusion:} Multi-decoder achieves same clustering performance as single decoder across wide $\beta$ range (ARI $\approx$ 0.186 $\pm$ 0.02), demonstrating architectural robustness with more consistent results.
\label{tab:beta_grid_v2_13}
\end{table}

\begin{table}[h]
\caption{Semi-Supervised VAE $\alpha$ Grid Search (v2.14)}
\centering
\begin{tabular}{@{}cccccc@{}}
\toprule
$\alpha$ & Classification Acc & GMM ARI & vs v2.6.7 & Active Dims & Status \\
\midrule
0.01 & --- & 0.248 & +26.5\% & 10/10 & Good \\
\textbf{0.1} & \textbf{$\sim$45\%} & \textbf{0.285} & \textbf{+45.6\%} & \textbf{10/10} & \textbf{Best} \\
0.5 & --- & 0.232 & +18.4\% & 10/10 & Good \\
1.0 & $\sim$55\% & 0.250 & +27.6\% & 10/10 & Too high \\
2.0 & $\sim$54\% & 0.220 & +12.2\% & 10/10 & Overfitting \\
\midrule
\multicolumn{6}{l}{\textit{Baseline (unsupervised):}} \\
v2.6.7 & N/A & 0.196 $\pm$ 0.037 & --- & 10/10 & Best unsupervised \\
\bottomrule
\end{tabular}
\vspace{0.5em}

\small
\textbf{Methodology:} Grid search over $\alpha \in \{0.01, 0.1, 0.5, 1.0, 2.0\}$ with fixed $\beta$ annealing (1e-10 $\rightarrow$ 0.75). Three-part loss: $\mathcal{L} = \mathcal{L}_{\text{recon}} + \beta \cdot \mathcal{L}_{\text{KL}} + \alpha \cdot \mathcal{L}_{\text{class}}$. Classification head: 10D latent $\rightarrow$ [32, ReLU, Dropout(0.2)] $\rightarrow$ 139 classes. Training: 100 epochs, batch size 1,024, 238,506 samples (same dataset as v2.6.7). Device: GPU.

\textbf{Key findings:}
\begin{itemize}
\item \textbf{All $\alpha$ values improve over unsupervised:} Even gentle supervision ($\alpha=0.01$) achieves +26.5\% improvement
\item \textbf{Sweet spot at $\alpha=0.1$:} Balances reconstruction, regularization, and classification. Achieves +45.6\% improvement
\item \textbf{Inverted-U relationship:} Performance peaks at $\alpha=0.1$, then declines as supervision dominates
\item \textbf{Too much supervision counterproductive:} $\alpha=2.0$ achieves higher classification accuracy ($\sim$54\%) but worse clustering (ARI=0.220) than $\alpha=0.1$ (acc=$\sim$45\%, ARI=0.285). Model overfits to classification task
\item \textbf{No posterior collapse:} All 10 latent dimensions remain active across all $\alpha$ values
\item \textbf{Single split only:} These results from single train-test split. Cross-validation needed to confirm improvement holds across folds
\end{itemize}

\textbf{Why it works:} Classification loss organizes latent space by encouraging lithologically similar samples to cluster together. GMM clustering (unsupervised) then benefits from better-separated clusters without needing labels. This is \textbf{guided representation learning}, not supervised classification---we use labels to train a better encoder, then evaluate with unsupervised clustering.

\textbf{Comparison to architectural innovations:}
\begin{itemize}
\item v2.1 (distribution-aware scaling): +30\% improvement over v2
\item v2.6.7 (extreme $\beta$ annealing): +18\% improvement over v2.1
\item v2.14 (semi-supervision $\alpha=0.1$): +45.6\% improvement over v2.6.7
\item \textbf{Total improvement:} +171\% from v2 to v2.14
\end{itemize}

\textbf{Future work:} (1) $\alpha$ annealing: start $\alpha=0$ (pure autoencoder) for 50 epochs, anneal to $\alpha_{\text{end}}$ to combine benefits of unsupervised pre-training with supervised guidance. (2) 5-fold entropy-balanced CV to confirm robustness. (3) Hierarchical classification using lithology taxonomy.

\textbf{Recommendation:} If labels available, use semi-supervised v2.14 ($\alpha=0.1$). For fully unsupervised, use v2.6.7.

\label{tab:semisup_alpha_grid}
\end{table}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{vae_v2_13_multidecoder_architecture_diagram.png}
\caption{\textbf{Figure S7. Multi-Decoder VAE v2.13 Architecture.} Complete data flow from raw input to reconstructed output. Key architectural features: (1) \textbf{Shared encoder:} 6D input $\rightarrow$ [32, 16] $\rightarrow$ 10D latent (identical to v2.6.7). Distribution-aware scaling applies median-IQR normalization with feature-specific transforms. (2) \textbf{Latent space:} Reparameterization $z = \mu + \sigma \cdot \epsilon$ produces 10D latent representation (collapses to $\sim$4D effective). (3) \textbf{Multi-decoder architecture:} Six separate decoder branches, one per feature (GRA, MS, NGR, R, G, B). Each decoder: 10D $\rightarrow$ [16, 32] $\rightarrow$ 1D output. Feature-specific decoders allow specialization for heteroskedastic reconstruction patterns. (4) \textbf{Feature weighting:} Loss function applies weights [1.0, 2.0, 2.0, 1.0, 1.0, 1.0], with 2$\times$ emphasis on MS and NGR (challenging features with lower baseline $R^2$). (5) \textbf{Inverse scaling:} Denormalization back to original measurement scales. \textbf{Performance:} 5,610 parameters (+179\% vs v2.6.7). Training: $\beta$ annealing 1e-10 $\rightarrow$ 0.75 over 50 epochs, 1,471~s on GPU. Entropy-balanced 5-fold CV: ARI = 0.187 $\pm$ 0.045 (statistically equivalent to v2.6.7's 0.196 $\pm$ 0.037). Reconstruction quality: GRA $R^2 = 0.69$, MS $R^2 = 0.48$, NGR $R^2 = 0.72$, RGB $R^2 = 0.87$--0.89. \textbf{Key finding:} Multi-decoder achieves same clustering as single decoder (latent space structure determines clustering) while maintaining comparable reconstruction with added decoder flexibility. Demonstrates architectural robustness: clustering performance independent of decoder architecture.}
\label{fig:multidec_arch}
\end{figure}
