"""
Test β annealing from essentially 0 (pure autoencoder) to 1.0 (standard VAE).

Current best: β: 0.001 → 0.5, ARI = 0.258 (single split)
Hypothesis: Starting from β ≈ 0 might learn better reconstructions early,
           but ending at β=1.0 may force too much disentanglement vs β=0.5.
"""
...
print("="*100)
print("β ANNEALING: 1e-10 → 1.0 TEST")
print("="*100)
print()
...
schedules = [
    ...
    {'name': '1e-10 → 0.5 (50ep)', 'beta_start': 1e-10, 'beta_end': 0.5, 'anneal_epochs': 50},
    {'name': '1e-10 → 0.5 (75ep)', 'beta_start': 1e-10, 'beta_end': 0.5, 'anneal_epochs': 75},
    {'name': '1e-10 → 0.5 (100ep)', 'beta_start': 1e-10, 'beta_end': 0.5, 'anneal_epochs': 100},
    {'name': '1e-10 → 0.75', 'beta_start': 1e-10, 'beta_end': 0.75, 'anneal_epochs': 50},
    {'name': '1e-10 → 1.0', 'beta_start': 1e-10, 'beta_end': 1.0, 'anneal_epochs': 50},
]
...
# Summary
print("="*100)
print("β ANNEALING SUMMARY")
print("="*100)
print()

df_results = pd.DataFrame(results)
print(df_results[['schedule', 'beta_end', 'anneal_epochs', 'best_ari', 'best_sil', 'train_time']].to_string(index=False))
print()

best_idx = df_results['best_ari'].idxmax()
baseline_idx = 0  # First is baseline

print(f"Baseline (0.001 → 0.5, 50ep): ARI = {df_results.loc[baseline_idx, 'best_ari']:.4f}")
...
print("Analysis:")
print(f"  Starting at β=1e-10 vs β=0.001: {'Helps' if df_results.loc[1, 'best_ari'] > df_results.loc[0, 'best_ari'] else 'No difference'}")
print(f"  β_end=0.5 vs β_end=1.0: {df_results[df_results['beta_end'] == 0.5]['best_ari'].max():.4f} vs {df_results[df_results['beta_end'] == 1.0]['best_ari'].values[0]:.4f}")
print(f"  Longer annealing (100ep vs 50ep): {'Helps' if df_results.loc[3, 'best_ari'] > df_results.loc[1, 'best_ari'] else 'No benefit'}")
print()

if df_results[df_results['beta_end'] == 1.0]['best_ari'].values[0] < df_results[df_results['beta_end'] == 0.5]['best_ari'].max():
    print("✓ Confirmed: β_end=0.5 > β_end=1.0 for clustering")
    print("  Lower β preserves feature correlations needed for clustering")
else:
    print("⚠️  Unexpected: β_end=1.0 performs as well or better than 0.5!")

print()
print("="*100)

df_results.to_csv('beta_1e10_to_1_results.csv', index=False)
print("Results saved to: beta_1e10_to_1_results.csv")
