"""
Create VAE GRA v2.3 Dataset - Adding Relative Depth to v2.1 features

This script creates a training dataset with 7 features:
...
- Relative depth (0-1, normalized within each borehole)
...
def add_relative_depth(df):
    """Add relative depth feature normalized 0-1 within each borehole."""
    print("\nAdding relative depth feature...")

    # For each borehole, normalize depth to 0-1
    def normalize_depth(group):
        depth_min = group['Depth_Bin'].min()
        depth_max = group['Depth_Bin'].max()

        # Handle case where borehole has only one depth (avoid division by zero)
        if depth_max == depth_min:
            group['Relative_Depth'] = 0.5  # Middle value
        else:
            group['Relative_Depth'] = (group['Depth_Bin'] - depth_min) / (depth_max - depth_min)

        return group

    df = df.groupby('Borehole_ID', group_keys=False).apply(normalize_depth)

    print(f"  Relative depth range: [{df['Relative_Depth'].min():.3f}, {df['Relative_Depth'].max():.3f}]")
    print(f"  Relative depth mean: {df['Relative_Depth'].mean():.3f}")
    print(f"  Relative depth std: {df['Relative_Depth'].std():.3f}")

    return df

def main():
    print("="*80)
    print("VAE GRA v2.3 DATASET CREATION - Adding Relative Depth")
    print("="*80)
    ...
    output_path = Path('/home/utig5/johna/bhai/vae_training_data_v2_3_20cm.csv')
    ...
    # Add relative depth
    print("\n6. Adding relative depth...")
    merged = add_relative_depth(merged)
